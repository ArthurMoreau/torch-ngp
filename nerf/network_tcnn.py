...
class NeRFWNetwork(NeRFRenderer):
    def __init__(self,
        ...
        self.in_dim_color = self.in_channels_a + self.encoder_dir.n_output_dims + self.geo_feat_dim
    ...
    def forward(self, x, d, l_a, l_t):
        ...
        #h = torch.cat([d, geo_feat, l_a], dim=-1)
        #h = torch.cat([d, geo_feat], dim=-1)
        h_s = torch.cat([d, geo_feat, l_a], dim=-1)
        h_s = self.color_net(h_s)
        
        # sigmoid activation for rgb
        color = torch.sigmoid(h_s)
...
class NeRFNetwork(NeRFRenderer):
    # will pass all elements in NeRFWNetwork but not passing l_a in forward
    def __init__(self,
                 encoding="HashGrid",
                 encoding_dir="SphericalHarmonics",
                 num_layers=2,
